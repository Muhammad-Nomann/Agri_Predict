...
#     X_class = df_supply[['Date', 'Commodity', 'Supply City', 'Quantity']]
#     X_class['Date'] = X_class['Date'].map(pd.Timestamp.toordinal).astype(np.float64)
...
#     X_reg = df_merged[['Date', 'Crop', 'City', 'Quantity']]
#     X_reg['Date'] = X_reg['Date'].map(pd.Timestamp.toordinal).astype(np.int64)
#     y_reg = df_merged['Avg Price']

#     rf_classifier = RandomForestClassifier(n_estimators=random.random, random_state=42)
...
def main():
    ...
    with tab4:
        ...
        with col2:
            ...
            quantity_pred = st.number_input("How much to sell (1=100kg)", min_value=0, value=100)
        ...
        if st.button("Get Predictions"):
            ...
            if isinstance(predicted_price, str):
                ...
                st.write(f"**Predicted Average Price of 100kg:** {predicted_price:.2f} PKR")
